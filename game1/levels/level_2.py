...
def start_level():
    ...
    hero_y = HEIGHT // 2 + 250 - HERO_SIZE // 2  # Начальная позиция героя по Y
    ...
    while waiting:
        ...
        if hero_x < WIDTH//2-245:
            hero_x = WIDTH//2-245
        elif hero_x + HERO_SIZE > WIDTH//2+245:
            hero_x = WIDTH//2+245 - HERO_SIZE
        if hero_y < HEIGHT//2+5:
            hero_y = HEIGHT//2+5
        elif hero_y + HERO_SIZE > HEIGHT//2+495:
            hero_y = HEIGHT//2+495 - HERO_SIZE
        ...
        line_delay_timer += 1
        if line_delay_timer >= 150: # 5 секунд (5 секунд * 60 FPS = 300 кадров)
            ...
            if color_change_timer >= 20:  # 0,3 секунда (0,3 секунда * 60 FPS = 20 кадров)
